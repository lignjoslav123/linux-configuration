...
date_color='#ff3357'
    ...
    while true ; do
        ...
        date +$'date\t^fg('$date_color')%H:%M^fg('$date_color'), %Y-%m-^fg('$date_color')%d'
        sleep 1 || break
...
workspace_full='#ffffff'
workspace_emthy='#c0c0c0'
        ...
        for i in "${tags[@]}" ; do
            case ${i:0:1} in
                '#')
                    ...
                    ;;
                '+')
                    echo -n "^bg(#BCA668)^fg(#141414)"
                    ;;
                ':')
                    echo -n "^bg()^fg("$workspace_full")"
                    ;;
                '!')
                    echo -n "^bg(#AG0675)^fg(#441414)"
                    ;;
                *)
                    echo -n "^bg()^fg("$workspace_emthy")"
                    ;;
            esac
            if [ ! -z "$dzen2_svn" ] ; then
